...
exports.modifyMypage = async (req, res, next) => {
  ...
  try {
    if (!req.file) {
      await Profile.updateOne({ userID: user.userID }, { $set: { petName, petGender, petBirth, petBreed } });
    } else {
      await Profile.updateOne({ userID: user.userID }, { $set: { petImage: req.file.location, petName, petGender, petBirth, petBreed } });
    }
    res.status(200).json({
      success: '정보가 수정되었습니다.',
    });
