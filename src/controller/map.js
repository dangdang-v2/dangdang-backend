...
exports.showImage = async (req, res, next) => {
  try {
    ...
    const imgUrl = await Profile.findOne({ userID: user.userID }, { petImage: true, _id: false });
...
exports.showMap = async (req, res, next) => {
  const { user } = res.locals;
  try {
    const list = await Maps.find({ userID: user.userID }, { _id: true, createdAt: true, distance: true, time: true });
    const profileImage = await Profile.findOne({ userID: user.userID }, { petImage: true });
    if (!list) {
